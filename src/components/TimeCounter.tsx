...
export default function TimeCounter({ visibleSections }: Props) {
  ...
  const fatimaBirth = new Date('2008-10-18T00:00:00');
  const mohamedBirth = new Date('2008-10-18T00:00:00');
    ...
    const interval = setInterval(() => {
      ...
      const calcAge = (birth: Date) => {
        ...
        if (diff > 0) {
          ...
          return { years: Math.abs(d.getUTCFullYear() - 1970), months: d.getUTCMonth(), days: d.getUTCDate() - 1 };
        }
  ...
  return (
    ...
    >
      <div className="bg-white/70 backdrop-blur-md rounded-3xl shadow-lg border border-rose-pink/10 p-6 md:p-8">
        ...
        <div className="flex items-center justify-center space-x-4 rtl:space-x-reverse text-xs font-mono text-rose-pink/60 flex-wrap gap-2">
          ...
          <div className="flex items-center space-x-1 rtl:space-x-reverse">
            ...
            <span>فاطمة: {ageFatima.years} سنة</span>
          </div>
          <div className="w-px h-4 bg-rose-pink/20"></div>
          <div className="flex items-center space-x-1 rtl:space-x-reverse">
            <Gift className="w-3.5 h-3.5" />
            <span>محمد: {ageMohamed.years} سنة</span>
          </div>
        </div>
